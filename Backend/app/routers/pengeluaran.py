...
from app.utils.prediksi import retrain_model_from_db
...
def create_pengeluaran(
    ...
    try:
        retrain_model_from_db(data.kategori, db=db, user_id=current_user.id_user)
    except Exception as e:
        print(f"[WARNING] Gagal retrain model untuk kategori {data.kategori}: {e}")
...
@router.get("/per-bulan", response_model=List[PengeluaranOut], summary="Daftar pengeluaran berdasarkan nama bulan dan tahun tertentu")
def get_pengeluaran_per_bulan(
    ...
    hasil = db.query(Pengeluaran)\
        .filter(
            Pengeluaran.id_user == current_user.id_user,
            extract("month", Pengeluaran.tanggal) == bulan_ke,
            extract("year", Pengeluaran.tanggal) == tahun
        )\
        .order_by(Pengeluaran.tanggal.desc(), Pengeluaran.id_pengeluaran.desc())\
        .all()

    if not hasil:
        raise HTTPException(status_code=404, detail=f"Tidak ada pengeluaran ditemukan untuk bulan {bulan} tahun {tahun}")
